...
Talk anchors:
- carregar modelo: async _carregarModeloInimigo -> explain one-time load and cloning for instances
- _reposicionarInimigo: spawning logic (side, random height/z, velocity) and rotation to face movement
- atualizar: per-frame movement, firing cadence, bounding box updates, and destruction animation
- obterInimigosColisiveis: why return minimal data (id and box3) for collision checks

When presenting, emphasize separation of responsibilities: SistemaInimigos only manages enemies' state and exposes minimal API for collision and firing.
*/
...
function prepararMateriaisParaAnimacao(opcaoInimigo) {
  ...
  opcaoInimigo.traverse((no) => { // percorre toda a hierarquia do modelo para encontrar os materiais
  ...
  atualizar(deltaSegundos, tempoAtualMs, obterPosicaoJogadorMundo, criarTiroInimigo) {
    ...
    for (const inimigo of this.inimigos) {
      if (inimigo.estado === 'ativo') {
        ...
        //os valores de escala foram escolhidos por tentativa e erro para criar uma animacao visualmente interessante, o lerp 
        const progresso = Math.min(inimigo.tempoAnimacaoDestruicao / 0.5, 1); // progresso de
